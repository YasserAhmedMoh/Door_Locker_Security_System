...
int main(void){
	...
	TimerConfig T0_Configuration={CTC,F_CPU_1024,0,7813};
	...
	while(1){
		...
		if(command== '-'){
			...
			if(Valid){
				SECONDS_T0_MC2 = 0;
				while ( SECONDS_T0_MC2 <= 100)   { 	MOTOR_clockw;       }
				while ( SECONDS_T0_MC2 <= 200)	{   MOTOR_stop;			}
				while ( SECONDS_T0_MC2 <= 300)   {   MOTOR_anti_clockw;  }
				MOTOR_stop;
			}
			else if(!Valid){
				buzzer_start();
				SECONDS_T0_MC2 = 0;
				while ( SECONDS_T0_MC2 <= 600);
				buzzer_stop();
				break;
			}
		}
